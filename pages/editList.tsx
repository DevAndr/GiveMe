...
const EditList: NextPage = () => {
    ...
    const emptyWishLists = (<div>
        <i className="pi pi-exclamation-circle pr-2 text-primary-300"></i><label>Еще нет списка желаний</label>
    </div>)

    return (
        <>
            <MainLayout isHideMenu={false} isHideFooter={true}
                        meta={{
                            title: `Give Me: Управление списком${currentWishList && ` - ${currentWishList?.name}`}`,
                            description: ''
                        }}>
                <div className={style.editContainer}>
                    <Splitter className={`sliderSplitter`}>
                        <SplitterPanel className={style.listsPanel} size={30} minSize={10}>
                            {
                        ...
                        <SplitterPanel className={style.viewPanel} size={70} minSize={60}>
